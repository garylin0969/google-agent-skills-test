...
1.  **React Hooks**：使用 `useState`, `useEffect`, `useReducer`, `useContext` 等。
2.  **事件監聽**：包含 `onClick`, `onChange`, `onSubmit` 等互動屬性。
3.  **瀏覽器專用 API**：使用 `window`, `document`, `localStorage` 等。
4.  **Class Component**：(雖然應避免使用，但若必須使用則需為 Client)。
...
- **作法**：將需要互動的「小部分 UI」拆分為獨立的元件（例如 `<LikeButton />`），僅在該小元件檔案中加入 `'use client'`。
- **目的**：讓父層與周圍的 Layout 保持為 Server Component 進行渲染。

### 模式 B：Server Actions 整合

- 若需要處理表單提交或資料變更，優先使用 **Server Actions**，而非切換成 Client Component 打 API Route。
- Server Action 可以直接在 Server Component 中呼叫，或作為 Prop 傳遞給 Client Component。

## 3. 程式碼實作規範

當判斷必須使用 Client Component 時，請遵循以下格式：

1.  **宣告位置**：`'use client'` 必須位於檔案的**第一行**（在所有 import 之前）。
2.  **範例**：
...
const SearchBar = () => {
    const [query, setQuery] = useState(''); // 使用了 Hook，所以必須是 Client
